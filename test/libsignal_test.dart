...
void main() {
  ...
  test('libsignal group test', () async {
    await groupTest();
  });
}
...
Future<void> install() async {
  ...
  final sessionStore = storage.sessionStore;
  final preKeyStore = storage.preKeyStore;
  final signedPreKeyStore = storage.signedPreKeyStore;
  final identityStore = storage.identityKeyStore;
...
Future<void> groupTest() async {
  ...
  final sentAliceDistributionMessage = await aliceSessionBuilder.create(
    groupSender,
  );
  ...
  final ciphertextFromAlice = await aliceGroupCipher.encrypt(
    Uint8List.fromList(utf8.encode('Hello')),
  );
