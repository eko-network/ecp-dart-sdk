import 'package:ecp/src/core/utils/uuid.dart';
...
/// Represents a delivery acknowledgment activity. Note that this type is generated by lowering the server level delivered. This is not a type defined by protocol and cannot be sent.
class Delivered implements StableActivity {
  @override
  final ActivityBase base;
  final Uri object; // The ID of the Create activity being acknowledged
  ...
  /// Create a Delivered activity from a server Delivered activity
  /// Uses a random UUID since this is a fake activity not persisted by the client
  factory Delivered.fromServerDelivered(dynamic serverDelivered, Uri actor) {
    return Delivered(
      base: ActivityBase(
        id: _uuid.v7obj(), // Random UUID - not persisted
        to: actor, // The original sender
      ),
      object: Uri.parse(serverDelivered.object as String),
    );
  }
